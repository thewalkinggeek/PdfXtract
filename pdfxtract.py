...
class LoadingWindow(ctk.CTkToplevel):
    """
    A simple loading screen that shows while the main application is initializing.
    This is especially useful for compiled binaries that have a slow startup time.
    """
    def __init__(self, parent):
        ...
        center_x = int(parent.winfo_screenwidth() / 2 - window_width / 2)
        center_y = int(parent.winfo_screenheight() / 2 - window_height / 2)
        self.geometry(f"{window_width}x{window_height}+{center_x}+{center_y}")
        self.resizable(False, False)

        main_frame = ctk.CTkFrame(self, corner_radius=15, border_width=2)
        main_frame.pack(fill="both", expand=True, padx=5, pady=5)

        # --- Custom Drawn Title ---
        self.title_canvas = tk.Canvas(main_frame, width=200, height=40, highlightthickness=0)
        self.title_canvas.pack(pady=(15, 5))
        self._draw_loading_title(self.title_canvas)

        ctk.CTkLabel(main_frame, text="Initializing...").pack()
        self.progress_bar = ctk.CTkProgressBar(main_frame, mode="indeterminate")
        self.progress_bar.pack(padx=20, pady=(5, 20), fill="x")

    def _draw_loading_title(self, canvas):
        """Draws the stylized title on the canvas for the loading screen."""
...
class PdfXtract(ctk.CTkToplevel):
    def __init__(self, master):
        ...
        # Defer logging until after the mainloop has started
        self.after(50, lambda: self.log("Welcome to PdfXtract!"))
...
if __name__ == "__main__":
    ...
    # Create the main application instance. It will be hidden initially by its own __init__.
    app = PdfXtract(master=root)

    # Create and display the loading screen.
    loading_screen = LoadingWindow(app)
    app.update_idletasks() # Ensure window state is updated
    loading_screen.progress_bar.start()

    # This function will be called after a short delay.
    # It destroys the loading screen and shows the main app window.
    # This gives the OS time to render the loading screen before the main app
    # potentially freezes the event loop during its own setup.
    def show_main_window():
        loading_screen.destroy()
        app.center_window() # Explicitly call center_window to show the main app
    ...
    # Schedule the main window to appear after a delay
    app.after(500, show_main_window)
